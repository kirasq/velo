    ...
    <div className="px-3 py-2 border-b border-border-secondary bg-accent/5">
      ...
      <div className="flex items-center gap-2 mb-2">
        <input
          ...
          placeholder={isReplyMode ? t("composer.ai.instructionsReply") : t("composer.ai.instructions")}
        ...
        {isReplyMode ? (
          ...
          >
            {loading ? "..." : t("composer.ai.generateReply")}
          </button>
        ) : (
          <button
            ...
            className="px-2 py-1 text-xs bg-accent text-white rounded hover:bg-accent-hover transition-colors disabled:opacity-50 flex items-center gap-1"
          >
            {loading ? "..." : t("composer.ai.generate")}
          </button>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5">
        <span className="text-xs text-text-tertiary mr-1">{t("composer.ai.transform")}</span>
        <QuickAction
          icon={<Wand2 size={11} />}
          label={t("composer.ai.improve")}
          onClick={() => handleTransform("improve")}
          disabled={loading}
        />
        <QuickAction
          icon={<ArrowDown size={11} />}
          label={t("composer.ai.shorter")}
          onClick={() => handleTransform("shorten")}
          disabled={loading}
        />
        <QuickAction
          icon={<Briefcase size={11} />}
          label={t("composer.ai.formal")}
