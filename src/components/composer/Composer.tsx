...
export function Composer() {
  ...
  const modeLabel =
    mode === "reply"
      ? t("composer.modeReply")
      : mode === "replyAll"
        ? t("composer.modeReplyAll")
        : mode === "forward"
          ? t("composer.modeForward")
          : t("composer.modeNewMessage");
    ...
    <div ref={overlayRef} className={`fixed inset-0 z-50 flex ${isFullpage ? "items-stretch justify-center p-4" : "items-end justify-center pb-4"} pointer-events-none`}>
      ...
      >
        ...
        <div className="px-3 py-2 space-y-1.5 border-b border-border-secondary">
          ...
          />
          <AddressInput label={t("composer.labelTo")} addresses={to} onChange={setTo} />
          {showCcBcc ? (
            <>
              <AddressInput label={t("composer.labelCc")} addresses={cc} onChange={setCc} />
              <AddressInput label={t("composer.labelBcc")} addresses={bcc} onChange={setBcc} />
            </>
          ) : (
            <button
