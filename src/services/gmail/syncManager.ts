...
import { Pop3Provider } from "../pop3/pop3Provider";
...
/**
 * Sync a POP3 account: download new messages via the Pop3Provider abstraction.
 * POP3 has no server-side folders/labels and uses UIDL-based dedupe, so we
 * always run a full pull (the provider handles incremental via known UIDLs).
 */
async function syncPop3Account(accountId: string): Promise<void> {
  const account = await getAccount(accountId);
  if (!account) {
    throw new Error("Account not found");
  }
  ...
  const provider = new Pop3Provider(accountId);
  await provider.initialSync(syncDays, (phase, current, total) => {
    statusCallback?.(accountId, "syncing", {
      // POP3 phases map loosely onto the UI's known phases
      phase: phase === "download" ? "messages" : "threads",
      current,
      total,
    });
  });
