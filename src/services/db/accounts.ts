...
export interface DbAccount {
  ...
  pop3_security?: string | null;
  pop3_retention_days?: number | null;
}
...
export async function insertPop3Account(account: {
  id: string;
  email: string;
  displayName: string | null;
  avatarUrl: string | null;
  ...
  const db = await getDb();
  const encPassword = await encryptValue(account.password);
  // POP3 reuses the imap_password / imap_username columns for credential storage.
  await db.execute(
    `INSERT INTO accounts (id, email, display_name, avatar_url, access_token, refresh_token, provider, pop3_host, pop3_port, pop3_security, smtp_host, smtp_port, smtp_security, auth_method, imap_password, imap_username, pop3_retention_days, accept_invalid_certs)
     VALUES ($1, $2, $3, $4, NULL, NULL, 'pop3', $5, $6, $7, $8, $9, $10, 'password', $11, $12, $13, $14)`,
    [
      account.id,
      account.email,
      account.displayName,
      ...
      account.pop3Host,
      account.pop3Port,
      account.pop3Security,
      account.smtpHost,
      account.smtpPort,
      account.smtpSecurity,
      encPassword,
      account.username,
      account.retentionDays,
      account.acceptInvalidCerts ? 1 : 0,
    ],
  );
}
...
/**
 * Update an existing POP3 account's connection settings.
 * Password is optional: pass `null` to keep the existing stored password.
 * POP3 reuses the imap_password / imap_username columns for credentials.
 */
export async function updatePop3Account(account: {
  id: string;
  email?: string | null;
  displayName?: string | null;
  pop3Host: string;
  ...
  const sets: string[] = [
    ...
    "smtp_security = $7",
    "imap_username = $8",
    "pop3_retention_days = $9",
    "accept_invalid_certs = $10",
  ...
  if (account.displayName !== undefined) {
    ...
    params.push(account.displayName);
    pi++;
  }
  if (encPassword !== null) {
    sets.push(`imap_password = $${pi}`);
