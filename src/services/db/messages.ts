...
export async function upsertMessage(msg: {
  ...
  const db = await getDb();
  await db.execute(
    `INSERT INTO messages (id, account_id, thread_id, from_address, from_name, to_addresses, cc_addresses, bcc_addresses, reply_to, subject, snippet, date, is_read, is_starred, body_html, body_text, body_cached, raw_size, internal_date, list_unsubscribe, list_unsubscribe_post, auth_results, message_id_header, references_header, in_reply_to_header, imap_uid, imap_folder, pop3_uidl)
     ...
     ON CONFLICT(account_id, id) DO UPDATE SET
       ...
       pop3_uidl = COALESCE($28, pop3_uidl)`,
    [
      msg.id,
      msg.accountId,
      msg.threadId,
